...
@Component({
  selector: "ds-kware-citation-section",
  standalone: true,
  imports: [
    NgIf,
    NgFor,
  ...
  templateUrl: "./kware-citation-section.component.html",
  styleUrl: "./kware-citation-section.component.scss",
})
export class KwareCitationSectionComponent {
  @Input() object: Item;
  date = new Date();
  ...
  this.selectedContent=this.convertComma(this.getCitationOfPublication()[0][1]);
  }

  getCitationOfPublication(): any {
    const authorData = this.object
      ?.allMetadataValues("dc.contributor.author")
      ?.map((author) => {
        return {
          given: this.convertComma(
            this.localeService.getStringByLocale(
              author?.split(",")[1]?.split(",undefined")[0]?.split(" null")[0]
            )
          ),
          family: this.convertComma(
            this.localeService.getStringByLocale(
              author?.split(",")[0]?.split(",undefined")[0]?.split(" null")[0]
            )
          ),
        };
      });
    ...
    const bibliographyData = [
      {
        id: this.object?.uuid,
        type: this.localeService.getStringByLocale(
          this.object?.firstMetadataValue("dc.type")
        ),
        title: this.convertComma(
          this.localeService.getStringByLocale(
            this.object?.firstMetadataValue("dc.title")
          )
        ),
        ...
        "container-title": this.convertComma(
          this.localeService.getStringByLocale(
            this.object?.firstMetadataValue("journal.title")
          )
        ),
        volume: this.convertComma(
          this.localeService.getStringByLocale(
            this.object?.firstMetadataValue("journalvolume.identifier.name")
          )
        ),
        issue: this.convertComma(
          this.localeService.getStringByLocale(
            this.object?.firstMetadataValue("publicationissue.title")
          )
        ),
        publisher: this.convertComma(
          this.localeService.getStringByLocale(
            this.object?.firstMetadataValue("dc.publisher")
          ) ||
            this.localeService.getStringByLocale(
              this.object?.firstMetadataValue("dc.source")
            )
        ),
        //  page: "5441-5444"
      },
  ...
  onSelect(event:any){
    this.selectedContent=event.target.value;
  }
