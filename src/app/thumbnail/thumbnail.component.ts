...
import { FileMetadataService } from "../shared/kware-media-viewer/services/file-metadata.service";
...
export class ThumbnailComponent implements OnChanges {
  ...
  srcUrl= new BehaviorSubject(undefined);
  ...
  ngOnInit(): void {
    // this.httpClient.get(this.item?._links?.thumbnail?.href).subscribe((res)=>{
    //   this.srcUrl.next(res['_links']?.content?.href)
    // })
    // this.httpClient.get(this.item._links.thumbnail.href).pipe(
    //   map((thumbnail: any) => {
    //     console.log(thumbnail);
    //     this.srcUrl.next(thumbnail?._thumbnail?._links?.content?.href)
    //     return thumbnail?._thumbnail;
    //   }),
    //   concatMap((res: any) => {
    //     if (res) {
    //       return this.httpClient.get(`${res?._links?.content?.href}`);
    //     } else {
    //       return undefined;
    //     }
    //   })
